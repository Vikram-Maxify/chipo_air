...
const Flights = () => {
  ...
  useEffect(() => {
    const fetchDelhiToMumbaiFlights = async () => {
      // Agar already fetched hai to mat karo
      if (hasAutoFetched.current) return;
      
      // Get current date + 1 day (tomorrow)
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      const defaultDate = format(tomorrow, "yyyy-MM-dd");
      
      // Get from URL or use default DEL to BOM
      const fromAirport = fromQuery || "DEL";
      const toAirport = toQuery || "BOM";
      const travelDate = departureDateQuery || defaultDate;
      
      console.log("Auto-fetching flights from DEL to BOM:", {
        from: fromAirport,
        to: toAirport,
        date: travelDate
      });
      
      // Set the from and to display values
      ...
      try {
        ...
        if (getFlightsThunk.fulfilled.match(resultAction)) {
          console.log("Auto-fetch successful! Loaded flights from", fromAirport, "to", toAirport);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-700 font-semibold text-lg">Loading Flights from Delhi to Mumbai...</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-9">
            ...
            {filteredFlights.length === 0 && !loading && !isSearchLoading ? (
